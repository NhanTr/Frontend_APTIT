...
const navConfig = {
  ...
  manager: [
    { key: "dashboard", label: "Tổng quan", icon: <LayoutDashboard className="size-4" /> },
    { key: "activity-approvals", label: "Thống kê hoạt động", icon: <CalendarDays className="size-4" /> },
...
export function AppSidebar({ isOpen = true, onClose, onMenuClick, activeMenu, role }) {
  const handleClose = () => {
    if (onClose) {
      onClose()
    }
  ...
  // For non-guest users, prepare user info
